...
(() => {
  ...
  async function action(where, fn) {if(busy)return;busy=true;controls();try{await fn();}catch(e){feedback(where,e.message);}finally{busy=false;controls();}}
  ...
  async function runtime(){const r=await api('/runtime');$('runtime').textContent=r.available?(r.authenticated?'Codex 已连接':'需要登录 Codex'):r.message;$('login').hidden=r.authenticated;}
  ...
  $('new').addEventListener('click',()=>action('input',async()=>{current=null;fillPolicy(defaults);await ensureTask();render();}));
  $('history').addEventListener('change',()=>action('input',async()=>{if($('history').value){current=await api('/'+$('history').value);fillPolicy(current.policy);history.replaceState(null,'','?task='+current.id);}else{current=null;fillPolicy(defaults);await ensureTask();}render();}));
  ...
  controls();
  (async()=>{try{await loadHistory();const id=new URL(location.href).searchParams.get('task');if(id){current=await api('/'+id);fillPolicy(current.policy);$('history').value=id;render();}else fillPolicy(defaults);await runtime();}catch(e){feedback('input',e.message);}finally{busy=false;controls();}})();
})();
