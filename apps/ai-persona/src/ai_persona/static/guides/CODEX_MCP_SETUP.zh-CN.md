...
> 请按这份文档帮我配置 PersonaStudio MCP，连接我正在使用的真实工作区。检查实际 Codex 环境，备份并保留已有配置，避免重复添加。完成后从目标 Codex 调用接入验证工具，确认连接到了正确工作区。沿用我的工具权限设置，暂不安装 Hook 或开启其他自动功能。
...
MCP 让 Codex 按需查询 Persona 的知识、偏好和材料，也提供待审核提案工具。它不要求安装 Hook；日常自动应用偏好、对话学习各自启用，不属于本次配置任务。普通接入检查不需要模型账号。
...
- 同一条目需要更新路径：保留原有环境、超时、工具限制和权限策略，只修改必要字段。
...
解析 Studio 返回的 `config`，核对其中的绝对 Python 路径、`--data`、`--state` 是否存在并指向目标工作区。解释器必须能够加载当前版本 `ai_persona.mcp_server`，不依赖终端的当前目录、临时环境或一段未保存的启动脚本。
...
名称、程序和路径均替换为上一步确认的值。程序调用优先传参数数组，使用 Shell 时正确引用含空格或特殊字符的路径。已有同名条目、项目级配置或需要保留条目额外字段时，使用保留格式的定点 TOML 编辑，不用 `mcp add` 覆盖整个已有条目。
...
| 请求 | 用途 |
| --- | --- |
| `GET /api/studio/mcp-setup` | 获取配置片段及当前检测／验证状态 |
| `POST /api/studio/mcp-setup/probe`，JSON 为 `{}` | 生成 15 分钟内有效的验证消息 |
...
1. **配置修改完成后**生成新验证消息。按返回的 `message`，从用户要接入的 Codex 调用该服务的 `verify_persona_connection` 工具，传入原样的 `code`。
2. 当前执行任务就是目标 Codex，且新工具已可用时，直接调用。否则把完整消息交给用户，请其在重新加载后的目标会话发送。不要另启动测试脚本、直接调用 Python 内部函数或修改验证数据库来冒充目标客户端调用。
3. 检查工具调用成功且返回 `workspace_id` 与第 1 步一致。按客户端展示方式读取返回结构；有的客户端把内容包在 `result` 中。错误返回或调用到了另一工作区不能算成功。
4. 再次读取 `/api/studio/mcp-setup`。同一主机与配置环境下，应看到本次对应的 `verified: true` 和 `verification.received`。旧的成功记录不代替本次验证；配置修改或过期后应重新生成消息。

当前 Studio 的配置检测读取 **Studio 进程所见的 CODEX_HOME 下的用户配置**，不能完整检查另一客户端的自定义目录、项目配置或远端配置。`configured` 只表示在那里发现了匹配路径的条目；`verification.host` 是执行 MCP 服务的主机，不是客户端身份凭证。状态未同步时，检查两个进程的配置目录，报告检测范围及真实调用结果，不为了让指示灯变绿而复制配置到错误位置。

验证工具只写入接入检查记录，不读取个人正文，不调用 Persona 模型，也不开启学习。Codex 处理任务本身仍使用其正常模型。普通知识检索可能使用本地语义模型，首次下载与模型驱动的偏好场景判定另行验证。

### 6. 交付与撤销

报告实际工作区、目标客户端与配置目录、服务名、修改与备份位置，以及三个独立结果：配置已写入、客户端已加载、工具调用已验证。附上对应的工作区 ID 和本次验证时间。需要重新加载或等待用户发验证消息时，明确下一步，不声称已经全部完成。

本次不自动安装 Hook、开启对话学习、调用模型测试或生成修改提案。用户另行要求这些功能时按其选择执行；MCP 不提供直接批准或发布候选的工具。
