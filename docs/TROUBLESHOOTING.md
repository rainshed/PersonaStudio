...
| Demo or Studio opens on a different port | Without `--port`, `setup` and `start` prefer real-workspace port 8765 or Demo port 8766 and automatically choose an available port if occupied. Follow the opened address or check `status`; unrelated applications or gateways can keep running. Only one Studio runs; use the workspace menu to switch, or stop it before a command-line switch. / 其他应用占用端口时会自动避开；Studio 只能运行一个，可在工作区菜单中切换，命令行切换前先停止当前服务。 |
